/**
 * Build the public landing site for GitHub Pages.
 *
 * Env:
 *   BASE_PATH  public path prefix (default `/handymd/` for project Pages)
 *   OUTDIR     output directory (default `example-dist` — kept for CI paths)
 */
...
const result = await Bun.build({
  ...
  minify: true,
  splitting: true,
  // Landing page never uses shiki — keep it out of the graph entirely.
  external: ['shiki'],
  naming: {
    entry: '[name].[ext]',
...
// CSS emitted from playground import of src/style.css — keep as handymd.css
...
if (cssOut) {
  ...
  await cp('src/style.css', join(outdir, 'handymd.css'))
}
...
// ——— 2) Static assets ———
await cp('site/styles.css', join(outdir, 'styles.css'))
await cp('site/favicon.svg', join(outdir, 'favicon.svg'))
await cp('site/sw.js', join(outdir, 'sw.js'))
await cp('site/manifest.webmanifest', join(outdir, 'manifest.webmanifest'))
...
// ——— 4) HTML: inject base + module entry (editor CSS loads with playground) ———
const htmlTemplate = await readFile('site/index.html', 'utf8')
...
// Ensure styles.css link stays (already in template)
await writeFile(join(outdir, 'index.html'), html)
await writeFile(join(outdir, '404.html'), html)

// Patch service worker PRECACHE is fine with relative paths under base scope.
