 ...
 * permanent 前缀的光标保护。
 *
 * 块级前缀（`- ` / `- [ ] ` / `> ` / `# ` / hr 整行）永久隐藏后，光标不应
 * 停留在这些不可见字符里 —— 点击行首、Home、跨行 ArrowRight 等都可能把
 * caret 放进隐藏区。本插件用 appendTransaction 把落入隐藏前缀的折叠光标
 * 推到内容起点（hr 推到行尾），保证"看不见的字符也走不进去"。
 *
...
export function permanentPrefixAt(
  ...
  for (const block of st.blocks) {
    ...
    for (const el of block.elements) {
      if (el.permanent && el.scope === 'block' && el.markers.length) return { block, el }
    }
    return null
  }
  return null
