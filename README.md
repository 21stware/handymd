...
- **标题不显示 `#`/`##`**：聚焦时在左侧 gutter 展示多级图标（`#`→一级、`##`→二级…，绝对定位不挤占文本）；失焦后图标消失。空标题行保留透明空格作为光标落脚点。
...
行内（conceal ⇄ reveal）：`**strong**`、`*em*`（可嵌套 strong）、`` `code` ``、`~~strike~~`、`==mark==`（高亮笔）、`[link](url)`、`![image](url)`、`#tag`（Bear 风格，永远 pill 展示）。

块级（permanent 渲染）：`# 标题`（1-6 级，聚焦时左侧层级图标）、`> 引用`、`- [ ] 待办`、`- 无序列表`、`1. 有序列表`、`---` 分隔线、``` 代码块围栏（可选 shiki 高亮）。
