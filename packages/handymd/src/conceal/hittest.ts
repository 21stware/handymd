...
export function isRevealed(el: ElementRange, sel: SelLike, readOnly: boolean): boolean {
  ...
  if (el.kind === 'image') return sel.from < sel.to && sel.from <= el.from && sel.to >= el.to
  if (sel.from === sel.to) return spansIntersect(sel.from, sel.to, el.hitFrom, el.hitTo)
  return (
    spansIntersect(sel.from, sel.from, el.hitFrom, el.hitTo) ||
    spansIntersect(sel.to, sel.to, el.hitFrom, el.hitTo)
  )
}
